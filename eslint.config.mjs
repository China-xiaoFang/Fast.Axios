...
export default defineConfig(
	...
	globalIgnores(
		...
		"fast-axios/ignores/global"
	),
	// 读取项目 `.gitignore`，补充仓库自己的忽略范围。
	{
		name: "fast-axios/ignores/git",
		...eslintConfigFlatGitignore({ strict: false }),
	},
	// 仅构建配置、插件入口与测试使用 Node.js 全局变量，请求核心保持平台中立。
	{
		name: "fast-axios/globals/node-tooling",
		files: ["*.config.{js,cjs,mjs,ts}", ".prettierrc.cjs", "src/unplugin/**/*.ts", "src/{vite,webpack}.ts", "tests/**/*.{js,mjs,ts}"],
		languageOptions: {
			globals: globals.node,
		},
	},
	// 请求 SDK 运行在浏览器、WebView 和 uni-app；uni 由运行平台注入，不是 npm 模块。
	{
		name: "fast-axios/globals/browser-uni",
		files: ["src/axios/**/*.ts", "src/uni-adapter/**/*.ts"],
		languageOptions: {
			globals: {
				...globals.browser,
				uni: "readonly",
	...
	// 跨 JavaScript 与 TypeScript 生效的公共规则。
	{
		name: "fast-axios/common",
		files: ["**/*.{js,cjs,mjs,ts}"],
		linterOptions: {
	...
	// TypeScript 本地覆写规则。
	{
		name: "fast-axios/typescript/type-checked",
		files: ["**/*.ts"],
		extends: [
			eslintJs.configs.recommended,
			{
				name: "fast-axios/typescript/javascript-rules",
				/**
				 * JavaScript 本地覆写规则。
				 *
				 * @remarks
	...
	{
		name: "fast-axios/sort/package-json",
		files: ["**/package.json"],
		/**
		 * package.json 属性排序规则。
		 *
	...
	{
		name: "fast-axios/markdown",
		files: ["**/*.md"],
		extends: [eslintMarkdown.configs.recommended],
	},
	// node:test 注册函数由测试运行器接管 Promise，声明为已知安全调用。
	{
		name: "fast-axios/node-test-registration",
		files: ["tests/**/*.test.ts"],
		rules: {
	...
	// Node.js 配置、插件入口与测试允许终端日志和 CommonJS 兼容加载。
	{
		name: "fast-axios/node-tooling",
		files: ["*.config.{js,cjs,mjs,ts}", ".prettierrc.cjs", "src/unplugin/**/*.ts", "src/{vite,webpack}.ts", "tests/**/*.{js,mjs,ts}"],
		rules: {
